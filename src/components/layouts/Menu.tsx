...
const AppMenu = ({ menuItems }: AppMenuProps) => {
  ...
  const handleLogout = async () => {
    try {
      const res = await fetch("/api/protected/auth/logout", {
        method: "GET",
      });
            ...
            <React.Fragment key={idx}>
              {item.isTitle ? (
                ...
                <li className="side-nav-item">
                  <button
                    className="side-nav-link bg-transparent border-0 text-start w-100 d-flex align-items-center gap-2"
                  ...
                  >
                    ...
                    {item.badge && (
                      <span
                        className={`badge bg-${item.badge.variant} ms-auto`}
                      >
                        {item.badge.text}
                      </span>
                    )}
